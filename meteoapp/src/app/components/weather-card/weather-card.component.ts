...
export class WeatherCardComponent {
  ...
  ngOnInit() {
    this.weather =
      this.WeatherService.fetchWeather(this.SelectedCountry()) ??
      ({} as Observable<Weather>);
    this.WeatherService.getCountryName(this.SelectedCountry())?.subscribe(
      (data) => {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['SelectedCountry']) {
      ...
      this.weather =
        this.WeatherService.fetchWeather(searchInput) ??
        ({} as Observable<Weather>);
      this.WeatherService.getCountryName(searchInput)?.subscribe((data) => {
        this.CountryName = data;
      });
    }
  }
